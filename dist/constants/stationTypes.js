...
exports.STATION_TYPES = void 0;
exports.getAllStationTypes = getAllStationTypes;
exports.getStationTypesByCategory = getStationTypesByCategory;
exports.getStationTypeInfo = getStationTypeInfo;
exports.isValidStationType = isValidStationType;
exports.getStationTypeCategories = getStationTypeCategories;
exports.suggestStationTypeForStation = suggestStationTypeForStation;
exports.getMusicStationTypes = getMusicStationTypes;
exports.getTalkNewsStationTypes = getTalkNewsStationTypes;
exports.isMusicStationType = isMusicStationType;
exports.isTalkNewsStationType = isTalkNewsStationType;
...
function getAllStationTypes() {
    return Object.keys(exports.STATION_TYPES);
}
function getStationTypesByCategory(category) {
    return Object.values(exports.STATION_TYPES).filter(type => type.category === category);
}
function getStationTypeInfo(type) {
    return exports.STATION_TYPES[type] || null;
}
function isValidStationType(type) {
    return type in exports.STATION_TYPES;
}
function getStationTypeCategories() {
    return ['music', 'talk', 'news', 'educational', 'entertainment', 'specialized', 'format'];
}
function suggestStationTypeForStation(station) {
    const suggestions = [];
    const searchText = `${station.name} ${station.description || ''} ${station.genre || ''} ${station.subgenre || ''}`.toLowerCase();
    Object.entries(exports.STATION_TYPES).forEach(([typeKey, typeInfo]) => {
        const hasKeyword = typeInfo.keywords.some(keyword => searchText.includes(keyword.toLowerCase()));
        if (hasKeyword) {
...
function getMusicStationTypes() {
    return Object.entries(exports.STATION_TYPES)
        .filter(([_, type]) => type.category === 'music')
        .map(([key, _]) => key);
}
function getTalkNewsStationTypes() {
    return Object.entries(exports.STATION_TYPES)
        .filter(([_, type]) => type.category === 'talk' || type.category === 'news')
        .map(([key, _]) => key);
}
function isMusicStationType(type) {
    const typeInfo = exports.STATION_TYPES[type];
    return typeInfo?.category === 'music' || false;
}
function isTalkNewsStationType(type) {
    const typeInfo = exports.STATION_TYPES[type];
