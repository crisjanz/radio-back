...
exports.COLLECTION_TAGS = void 0;
exports.getAllCollectionTags = getAllCollectionTags;
exports.getCollectionTagsByCategory = getCollectionTagsByCategory;
exports.getCollectionTagBySlug = getCollectionTagBySlug;
exports.isValidCollectionTag = isValidCollectionTag;
exports.getCollectionTagSlugs = getCollectionTagSlugs;
exports.getCollectionTagCategories = getCollectionTagCategories;
exports.suggestTagsForStation = suggestTagsForStation;
exports.getCurrentSeasonalTags = getCurrentSeasonalTags;
exports.COLLECTION_TAGS = {
    'morning-boost': {
        name: 'Morning Boost',
        slug: 'morning-boost',
...
function getAllCollectionTags() {
    return Object.values(exports.COLLECTION_TAGS);
}
function getCollectionTagsByCategory(category) {
    return Object.values(exports.COLLECTION_TAGS).filter(tag => tag.category === category);
}
function getCollectionTagBySlug(slug) {
    return Object.values(exports.COLLECTION_TAGS).find(tag => tag.slug === slug) || null;
}
function isValidCollectionTag(slug) {
    return slug in exports.COLLECTION_TAGS;
}
function getCollectionTagSlugs() {
    return Object.keys(exports.COLLECTION_TAGS);
}
function getCollectionTagCategories() {
    return ['time', 'activity', 'mood', 'seasonal', 'special'];
}
function suggestTagsForStation(station) {
    const suggestions = [];
    const searchText = `${station.name} ${station.description || ''} ${station.genre || ''} ${station.subgenre || ''}`.toLowerCase();
    Object.entries(exports.COLLECTION_TAGS).forEach(([slug, tag]) => {
        const hasKeyword = tag.keywords.some(keyword => searchText.includes(keyword.toLowerCase()));
        if (hasKeyword) {
...
function getCurrentSeasonalTags() {
    const now = new Date();
